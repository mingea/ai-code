...
public class WebScreenshotUtils {
    ...
    private static WebDriver initChromeDriver(int width, int height) {
        try {
            // 配置 WebDriverManager 使用国内镜像源
            System.setProperty("wdm.proxyImageUrl", "https://npm.taobao.org/mirrors/chromedriver");
            System.setProperty("wdm.driverUrl", "https://npm.taobao.org/mirrors/chromedriver");

            // 也可以使用以下镜像源（按优先级推荐）：
            // 1. 阿里云镜像：https://npm.taobao.org/mirrors/chromedriver
            // 2. 清华镜像：https://mirrors.tuna.tsinghua.edu.cn/chromedriver
            // 3. 华为镜像：https://mirrors.huaweicloud.com/chromedriver

            // 设置不使用缓存，强制从镜像源下载
            System.setProperty("wdm.useMirror", "true");

            // 设置日志级别为INFO，方便查看下载进度
            System.setProperty("wdm.logLevel", "INFO");
            // 自动管理 ChromeDriver
