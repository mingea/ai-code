...
import com.wzb.aicode.manager.CosManager;
...
public class MermaidDiagramTool {
     ...
     * 将Mermaid代码转换为SVG图片
     */
    private File convertMermaidToSvg(String mermaidCode) throws IOException {
        // 创建项目本地临时目录
        String projectTmpDir = System.getProperty("user.dir") + "/tmp";
        File tmpDir = new File(projectTmpDir);
        if (!tmpDir.exists()) {
            tmpDir.mkdirs();
        }

        // 创建临时输入文件
        File tempInputFile = File.createTempFile("mermaid_input_", ".mmd", tmpDir);
        FileUtil.writeUtf8String(mermaidCode, tempInputFile);

        // 创建临时输出文件
        File tempOutputFile = File.createTempFile("mermaid_output_", ".svg", tmpDir);
